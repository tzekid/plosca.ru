...
Tracking rule: each item below is planned, implemented, verified, committed,
pushed, and published before moving to the next coherent slice. "Published"
means the disk-backed production site has the updated static files or, for
server/build changes, the deployed binary/process has been refreshed.
...
| Content quality checks | quick win | completed | Extended `zig build check-site` with manifest sync checks, title validation, meta description validation, duplicate/empty ID checks, local fragment checks, manifest route checks, and empty-anchor checks that ignore commented markup and Pandoc code line anchors. | `zig build check-site`, `zig build test`, `zig build -Doptimize=ReleaseFast` | ready for slice commit/publish |
| Heading self-links | quick win | completed | Added `site-tool write` HTML enhancement for h2/h3 permalink anchors plus checker enforcement for article/prose pages. | `zig build css`, `zig build check-site` | ready for slice commit/publish |
| Better 404 recovery | quick win | completed | Reworked `404.html` with clearer recovery copy and visible links to home, about, article, and prose. | `zig build check-site`, generated output review | ready for slice commit/publish |
| Article related links | quick win | completed | Added quiet related-link sections to `/hello_world` and `/prose`, styled in the editorial system. | `zig build check-site`, generated output review | ready for slice commit/publish |
| Keep theme simple | quick win | completed | Left the existing session-lifetime `/theme.js` model in place and intentionally avoided a Gwern-style toolbar or persistent localStorage setting. | focused local browser review | ready for slice commit/publish |
| Generated backlinks | medium | completed | `zig build css` now generates `static/metadata/backlinks/*.html` and combined connection fragments from local in-page links. | `zig build css`, `zig build check-site` | ready for slice commit/publish |
| CSS-only sidenotes | medium | completed | Added `.footnotes`, `.sidenote`, and `.margin-note` styling that stays inline on small screens and can float into the right margin on wide article layouts. | `zig build check-site`, generated output review | ready for slice commit/publish |
| Native collapses | medium | completed | Added native `details`/`summary` article styling with keyboard-visible focus and no custom state code. | `zig build check-site`, generated output review | ready for slice commit/publish |
| Markdown/text alternates | medium | completed | Generated `/hello_world.md`, `/hello_world.txt`, `/prose.md`, and `/prose.txt`; added alternate links in article heads and `.md` content-type support in the Zig server. | `zig build test`, generated output review | ready for slice commit/publish |
| Link-type markers | medium | completed | Added CSS-only external and PDF markers scoped to article content, excluding TOC, heading anchors, related blocks, and button-style links. | `zig build check-site`, generated output review | ready for slice commit/publish |
| Annotation DB and previews | large | superseded | The HTMX 4 refactor removed the browser annotation database. Zig now emits one escaped HTML preview fragment per annotated link. | `zig build check-site`, generated output review | implemented locally |
| Client-side transclusion | large | superseded | The HTMX 4 refactor materializes connection views into the first page response; standalone fragments remain reusable but are not fetched on startup. | `zig build check-site`, generated output review | implemented locally |
...
| Local external-link archive registry | large | completed | Generated `static/metadata/external-links.json`, `/archive`, and one local archive-record page per external URL, without copying third-party content. | `zig build check-site`, generated output review | ready for slice commit/publish |
